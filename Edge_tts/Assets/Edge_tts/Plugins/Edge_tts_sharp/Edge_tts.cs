...
namespace Edge_tts_sharp
{
    public class Edge_tts
    {
        ...
        {
            return
                $"<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis'  xml:lang='{lang}'><voice name='{voice}'><prosody pitch='+0Hz' rate ='{FromatPercentage(rate)}' volume='{volume}'>{text}</prosody></voice></speak>";
        }

        static string ConvertToSsmlWebSocketString(string requestId, string lang, string voice, int rate, int volume,
            string msg)
        {
            return
        ...
        {
            Invoke(option, voice, (_binary) => { Audio.PlayToByteAsync(_binary.ToArray(), option.Volume); });
        }
        ...
        public static AudioPlayer GetPlayer(PlayOption option, eVoice voice)
        {
            ...
            Invoke(option, voice, (_binary) => { player = new AudioPlayer(_binary.ToArray(), option.Volume); });
            while (player == null)
            {
                Thread.Sleep(10);
            }
        ...
        public static List<eVoice> GetVoice()
        {
            ...
            var voiceList = Resources.Load<TextAsset>("Edge_tts_VoiceList");
            return Tools.StringToJson<List<eVoice>>(voiceList.text);
        }
    }
}